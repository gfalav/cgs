class Vano < ActiveRecord::Base
  attr_accessible :cond_e_id, :cond_g_id, :flechamax_e, :flechamax_g, :nombre, :proyecto_id, :tiromax_e, :tiromax_g, :tma_e, :tma_g, :vano
  ...
  def calcvano
    ...
    proyecto.zona.condclimas.each {|c|
      ...
      pv1 = pvcond(proyecto.zona_id, c.viento, 0, vano.cond_e.diametro,vano.vano,hconductor)
      ph1 = phcond(proyecto.zona_id,vano.cond_e.diametro,c.hielo)
      pt1 = Math.sqrt(pv1**2 + (ph1+vano.cond_e.peso/1000)**2)
      pang1 = Math.atan(pv1 / (vano.cond_e.peso/1000 + ph1)) / Math::PI * 180
      
      t1 = vano.cond_e.tmax * vano.cond_e.seccion
      tmax = t1
      if (c.nombre=="Tmed")
        tmed = t1 / vano.cond_e.seccion
      end
      f1 = vano.vano**2 * pt1 / 8 / t1
      fmax = f1
      
      k1 = vano.vano**2 * pt1**2 / 24 / (t1**2)  - vano.cond_e.coef_t * c.temp - t1 / vano.cond_e.coef_e / vano.cond_e.seccion
      ...
      calc.hielo = c.hielo
      calc.tension = t1 / vano.cond_e.seccion
      calc.tiro = t1
      calc.flecha_t = f1
      calc.flecha_h = f1 * Math.sin(pang1 / 180 * Math::PI)
      calc.flecha_v = f1 * Math.cos(pang1 / 180 * Math::PI)
      calc.conductor_id = vano.cond_e.id
      arrtmp << calc
      
      proyecto.zona.condclimas.each {|c2|
        if (c.id != c2.id)          
          pv2 = pvcond(proyecto.zona_id, c2.viento, 0, vano.cond_e.diametro,vano.vano,hconductor)
          ph2 = phcond(proyecto.zona_id,vano.cond_e.diametro,c2.hielo)
          pt2 = Math.sqrt(pv2**2 + (ph2+vano.cond_e.peso/1000)**2)
          pang2 = Math.atan(pv2 / (vano.cond_e.peso/1000 + ph2)) / Math::PI * 180
          ...
          if (t2 > t1)
            ...
            calc.hielo = c2.hielo
            calc.tension = t2 / vano.cond_e.seccion
            calc.tiro = t2
            calc.flecha_t = f2
    ...
    if (tmed > vano.cond_e.tmed)
      Condclima.where(:zona_id => proyecto.zona_id,:nombre=>'Tmed').each { |c|
        ...
        t1 = vano.cond_e.tmed * vano.cond_e.seccion
        tmax = t1
        tmed = t1 / vano.cond_e.seccion
        f1 = vano.vano**2 * pt1 / 8 / t1
        fmax = f1
